...
print_proposal_count <- function(props, filtered_props) {
  paste0(
    "<h5>All Programs Proposal Count Statistics</h5>There were ",
    length(unique(props$`solicitation id`)),
    " Earth Action solicitations between 2006-2022.<br>",
    nrow(props),
    " proposals were submitted.<br>",
    nrow(props[props$`proposal status` == "SELECTED",]),
    " were selected.<br>",
    "The selection percentage was ",
    round(nrow(props[props$`proposal status` == "SELECTED",]) / nrow(props) *
            100, 2),
    "%.<br><br><h5>Filtered Programs Proposal Count Statistics</h5>There were ",
    length(unique(filtered_props$`solicitation id`)),
    " filtered program solicitations between 2006-2022.<br>",
    nrow(filtered_props),
    " proposals were submitted.<br>",
    nrow(filtered_props[filtered_props$`proposal status` == "SELECTED",]),
    " were selected.<br>",
    "The selection percentage was ",
    round(nrow(filtered_props[filtered_props$`proposal status` == "SELECTED",]) /
            nrow(filtered_props) * 100, 2),
    "%.<br><br>"
  )
}
...
print_proposal_award_amount <- function(props, filtered_props) {
  paste0(
    ## wish list activity: export dollar amounts as MILLIONS, rounding to 2 if Millions, e.g., print("$0.56M", "$2.60M")
    "<h5>All Programs Proposal Award Amount Statistics</h5>The total amount awarded by NASA to selected Earth Action proposals between 2006-2022 was $",
    sum(props[props$`proposal status` == "SELECTED",]$`proposed amount total`, na.rm = TRUE),
    ".<br>The average amount per proposal awarded by NASA to selected Earth Action proposals during this time was $",
    round(mean(props[props$`proposal status` == "SELECTED",]$`proposed amount total`, na.rm = TRUE), 0),
    "<br><br><h5>Filtered Programs Proposal Award Statistics</h5>The total amount awarded by NASA to selected filtered program proposals between 2006-2022 was $",
    sum(filtered_props[filtered_props$`proposal status` == "SELECTED",]$`proposed amount total`, na.rm = TRUE),
    ".<br>The average amount per proposal awarded by NASA to selected filtered program proposals during this time was $",
    round(mean(
      filtered_props[filtered_props$`proposal status` == "SELECTED",]$`proposed amount total`, na.rm = TRUE
    ), 0),
    ".<br><br>"
  )
}
...
print_pi <- function(props, ppl, filtered_props, filtered_ppl) {
  ...
  ppl_selected <-
    ppl |> filter(proposal.number %in% selected_props$proposal.number)
  num_selected_props <-
  ...
  if (nrow(filtered_props) == 0) {
    ...
    filtered_ppl_selected <-
      filtered_ppl |> filter(proposal.number %in% filtered_selected_props$proposal.number)
...
print_org <- function(props, filtered_props) {
  ...
  if (nrow(filtered_props) == 0) {
    paste0(
      ...
      length(unique(props$linked.org)),
      " organizations were involved in proposal submissions.<br>",
      length(unique(selected_props$linked.org)),
      " organizations were involved in proposal selections.<br><br>",
      nrow(multiple_apps),
      " organizations were involved in multiple submissions.<br>",
      round(sum(orgs$submission.count > 1) / nrow(orgs) * 100, 2),
      "% of organizations were involved in multiple submissions.<br>",
      nrow(multiple_apps[multiple_apps$`selection.count` > 0,]),
      " organizations were involved in multiple selections<br>",
      round(nrow(multiple_apps[multiple_apps$`selection.count` > 0,]) / nrow(multiple_apps) * 100, 2),
      "% of organizations were involved in multiple selections.<br><br>"
    )
  } else {
    filtered_orgs <- get_org_df(filtered_props)
    filtered_multiple_apps <-
      filtered_orgs[filtered_orgs$submission.count > 1,]
    filtered_selected_props <-
      filtered_props[filtered_props$`proposal.status` == "SELECTED",]
    paste0(
      "<h5>All Programs Linked Organization Statistics</h5>",
      ...
      " organizations were involved in multiple selections<br>",
      round(nrow(multiple_apps[multiple_apps$`selection.count` > 0,]) / nrow(multiple_apps) * 100, 2),
      "% of organizations were involved in multiple selections.<br><br><h5>Filtered Programs Linked Organization Statistics</h5>",
      length(unique(filtered_props$linked.org)),
      " organizations were involved in proposal submissions.<br>",
      length(unique(filtered_selected_props$linked.org)),
      " organizations were involved in proposal selections.<br><br>",
      nrow(filtered_multiple_apps),
      " organizations were involved in multiple submissions.<br>",
      round(
        sum(filtered_orgs$submission.count > 1) / nrow(filtered_orgs) * 100,
        2
      ),
      "% of organizations were involved in multiple submissions.<br>",
      nrow(filtered_multiple_apps[filtered_multiple_apps$`selection.count` > 0,]),
...
print_participation <- function(props, filtered_props) {
  selected_props <- props[props$`proposal status` == "SELECTED",]
  international_submitted <-
    nrow(props[props$`international participation` == "true",])
  international_selected <-
    nrow(selected_props[selected_props$`international participation` == "true",])
  nasa_submitted <- nrow(props[props$`nasa staff` == "true",])
  nasa_selected <-
    nrow(selected_props[selected_props$`nasa staff` == "true",])
  
  selected_filtered_props <-
    filtered_props[filtered_props$`proposal status` == "SELECTED",]
  filtered_international_submitted <-
    nrow(filtered_props[filtered_props$`international participation` == "true",])
  filtered_international_selected <-
    nrow(selected_filtered_props[selected_filtered_props$`international participation` == "true",])
  filtered_nasa_submitted <-
    nrow(filtered_props[filtered_props$`nasa staff` == "true",])
  filtered_nasa_selected <-
    nrow(selected_filtered_props[selected_filtered_props$`nasa staff` == "true",])
  
  paste0(
    "<h5>All Programs Participation Statistics</h5>",
    nasa_submitted,
    " submitted proposals had NASA Staff participation.<br>",
    round(nasa_submitted / nrow(props) * 100, 0),
    "% of submitted proposals had NASA staff participation.<br>",
    nasa_selected,
    " selected proposals had NASA Staff participation.<br>",
    round(nasa_selected / nrow(selected_props) * 100, 0),
    "% of selected proposals had NASA staff participation.<br><br>",
    international_submitted,
    " submitted proposals had international participation.<br>",
    round(international_submitted / nrow(props) * 100, 2),
    "% of submitted proposals had international participation.<br>",
    international_selected,
    " selected proposals had international participation.<br>",
    round(international_selected / nrow(selected_props) * 100, 2),
    "% of selected proposals had international participation.<br><br><h5>Filtered Programs Participation Statistics</h5>",
    
    filtered_nasa_submitted,
    " submitted proposals had NASA Staff participation.<br>",
    round(filtered_nasa_submitted / nrow(filtered_props) * 100, 0),
    "% of submitted proposals had NASA staff participation.<br>",
    filtered_nasa_selected,
    " selected proposals had NASA Staff participation.<br>",
    round(
      filtered_nasa_selected / nrow(selected_filtered_props) * 100,
      0
    ),
    "% of selected proposals had NASA staff participation.<br><br>",
    filtered_international_submitted,
    " submitted proposals had international participation.<br>",
    round(
      filtered_international_submitted / nrow(filtered_props) * 100,
      2
    ),
    "% of submitted proposals had international participation.<br>",
    filtered_international_selected,
    " selected proposals had international participation.<br>",
    round(
      filtered_international_selected / nrow(selected_filtered_props) * 100,
      2
    ),
    "% of selected proposals had international participation.<br><br>"
  )
}
